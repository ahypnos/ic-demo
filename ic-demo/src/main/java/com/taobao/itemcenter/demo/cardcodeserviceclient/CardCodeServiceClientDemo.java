...
public class CardCodeServiceClientDemo {
	...
	/**
	 * 查找卡密信息 调用getCardCodeInfoByQuery接口
	 * 
	 * @throws IcException
	 */
	...
	/**
	 * 分页查询卡密信息 调用getCardCodeInfoByQueryWithPage接口
	 * 
	 * @throws IcException
	 */
	public void getCardCodeInfoByQueryWithPage_分页查询卡密信息() throws IcException {
		...
		CardCodeQuery result = cardCodeServiceClient
				.getCardCodeInfoByQueryWithPage(query);
	...
	/**
	 * 修改同一笔订单的所有卡密状态,从已发送状态更新到已查看状态 调用updateCardCodeToViewedStateForQuery接口
	 * 
	 * @throws IcException
	 */
	public void updateCardCodeToViewedStateForQuery_修改同一笔订单的所有卡密状态()
			throws IcException {
		// 根据自己的需要的查询设置相应的条件
		tradeId = 4001117207L;
		itemId = 1500004274891L;
		...
		// 返回更新成功的卡密条数
		int counts = cardCodeServiceClient.updateCardCodeToViewedStateForQuery(
				tradeId, itemId, sellerNumId);
		System.out.println(String.format("更新卡密状态从已发送到已查看的记录有：%s条！", counts));
		printLine("updateCardCodeToViewedStateForQuery接口调用");
	}

	/**
	 * 恢复单个卡密至未发送状态,并更新对应商品库存 调用resetCardCode接口
	 * 
	 * @throws IcException
	 */
	public void resetCardCode_恢复单个卡密至未发送状态_并更新对应商品库存() throws IcException {
		...
		boolean result = cardCodeServiceClient.resetCardCode(tradeId, itemId,
				buyerId, sellerId, saleWinQuantity);
		System.out.println("重置卡密结果：" + result);
		printLine("resetCardCode接口调用");
	}

	/**
	 * 更新卡密信息，传递item时必须设置autoArea、sellerId、itemId三个字段 调用updateAutoCCInfo接口
	 * 
	 * @throws IcException
	 */
